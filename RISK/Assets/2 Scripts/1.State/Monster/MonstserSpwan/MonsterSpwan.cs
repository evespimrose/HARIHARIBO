...
public class MonsterSpwan : MonoBehaviour
{
    ...
    private Vector3 bossSpawnPoint; //Boss Monster Spwan Point
    ...
    public IEnumerator MonsterSpwanCorutine()
    {
        ...
        while (true)
        {
            ...
            while (curTime < SpwanTiem)
            {
                ...
                {
                    Debug.Log("All monsters are dead, restart respawn");
                    break;
                }
    ...
    public void Spwan(int quadrant)
    {
        Vector3 spwanPos;
        ...
        {
            spwanPos = GetSpwanPos(quadrant, meleeMinRadius, meleeMaxRadius);
            MeleeMonsterCreate(spwanPos);
        }

        for (int i = 0; i < currentSpawnSetting.rangeCount; i++)
        {
            spwanPos = GetSpwanPos(quadrant, rangeMinRadius, rangeMaxRadius);
            RangeMonsterCreate(spwanPos);
        }

        for (int i = 0; i < currentSpawnSetting.eliteCount; i++)
        {
            spwanPos = GetSpwanPos(quadrant, rangeMinRadius, rangeMaxRadius);
            EliteMonsterCreate(spwanPos);
        }

        for (int i = 0; i < currentSpawnSetting.structureCount; i++)
        {
            spwanPos = GetSpwanPos(quadrant, meleeMinRadius, meleeMaxRadius);
            StructureMonsterCreate(spwanPos);
        }

        for (int i = 0; i < currentSpawnSetting.bossCount; i++)
        {
            BossMonsterCreate(bossSpawnPoint);
        }
    }

    public Vector3 GetSpwanPos(int quadrant, float minRadius, float maxRadius)
    {
        float angle = 0f;
